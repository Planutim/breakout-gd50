...
function PaddleSelectState:init()
    -- the paddle we're highighting, will be passed to the ServeState
    -- when we press Enter
...
function PaddleSelectState:update(dt)
    ...
    -- select paddle and move on the the serve state, passsing in the selection
...
function PaddleSelectState:render()
    ...
    if self.currentPaddle == 1 then
        love.graphics.setColor(40/255,40/255, 40/255, 40/255, 128/255)
    end
  ...
  -- right arrow; should render normally if we're less than 4, else
    ...
    --draw the paddle itself, based on whic we have selected
